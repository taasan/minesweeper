...
export const getValue = (mask: mask, cell: number) =>
  (cell & (0xf << mask)) >> mask;
...
export const getMine = (cell: number): value =>
  getValue(mineMask, cell) as value;
...
export const getThreats = (cell: number): NumThreats =>
  getValue(threatsMask, cell) as NumThreats;
...
export const toObject = (cell: number): CellRecordObject => {
  ...
  return {
    state,
    threatCount: isNumThreats(threatCount) ? threatCount : undefined,
    mine: mine > 0 ? (mine as Mine) : undefined,
  };
};
